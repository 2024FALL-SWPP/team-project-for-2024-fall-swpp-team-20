...
public class EasyBookColorAnomaly : Anomaly
{
    ...
    public override void Apply(GameObject map)
    {
        for (int i = 0; i < 2; i++)
        {
            GameObject bookCollection = map.transform.Find("Interior").Find("2nd Floor").Find("Apartment_01").Find("Props").Find("book_collection" + i).gameObject;
            for (int j = 0; j < 8; j++)
            {
                bookCoverRenderer[j] = bookCollection.transform.Find("book" + "__" + j + "_").Find("book_cover").GetComponent<Renderer>();
